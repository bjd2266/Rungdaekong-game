...
function safeList(x) {
  ...
  return x.filter(e => e && cleanName(e.name) && cleanScore(e.score) !== null)
    .map(e => ({ name: cleanName(e.name), score: cleanScore(e.score), updatedAt: e.updatedAt || 0 }))
...
export default async (request) => {
  ...
  if (request.method === "GET") {
    ...
    return new Response(JSON.stringify({ leaderboard: leaderboard.slice(0, 10) }), { status: 200, headers: HEADERS });
  }
  ...
  try { body = await request.json(); } catch {
    return new Response(JSON.stringify({ error: "잘못된 요청입니다." }), { status: 400, headers: HEADERS });
  }
  ...
  if (!name || score === null || duration === null) {
    return new Response(JSON.stringify({ error: "닉네임/점수 형식 오류" }), { status: 400, headers: HEADERS });
  }

  // Basic plausibility check. The game earns roughly 7.8 pts/min before bonuses.
  // Allow generous headroom for boss bonuses and future tuning.
  const plausibleMax = Math.floor(duration * 2.5 + 1200);
  if (score > plausibleMax) {
    return new Response(JSON.stringify({ error: "점수 검증 실패" }), { status: 400, headers: HEADERS });
  }
  ...
  if (personalBest) {
    leaderboard = leaderboard.filter(x => x.name !== name);
    leaderboard.push({ name, score, updatedAt: Date.now() });
    leaderboard.sort((a,b) => b.score - a.score || a.updatedAt - b.updatedAt);
    leaderboard = leaderboard.slice(0, MAX_ENTRIES);
    await store.setJSON(KEY, { leaderboard });
  }

  const rank = leaderboard.findIndex(x => x.name === name) + 1;
